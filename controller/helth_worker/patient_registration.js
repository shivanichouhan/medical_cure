...
exports.accepted_patient_list = (req, res) => {
    const { doctor_id } = req.body
    const arr = []
    patient.find({ $and: [{ status: "accepted", doctor_id: doctor_id }] })
        .exec(async (err, List) => {
            if (err) {
                res.json({ code: 400, msg: 'patient list not found' })
            }
...
exports.acceptedPatient_data = (req, res) => {
    const { doctor_id, patient_id } = req.body
    patient.findOne({ $and: [{ _id: patient_id, doctor_id: doctor_id }] })
        .exec(async (err, List) => {
            if (err) {
                res.json({ code: 400, msg: 'patient list not found' })
            }
